...
public final class Board {
    ...
    public static final Board EMPTY = new Board(new PlacedTile[625], new int[0], ZonePartitions.EMPTY, new HashSet<>()); // Represents an empty board.
    ...
    public PlacedTile tileAt(Pos position) {
        return isPositionInBoard(position) ? placedTiles[posToIndex(position)] : null;
    }
    ...
    public Area<Zone.Meadow> adjacentMeadow(Pos pos, Zone.Meadow meadowZone) {
        Set<Zone.Meadow> adjacentMeadowZone = new HashSet<>();

        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                PlacedTile placedTile = tileAt(pos.translated(i, j));
                if (placedTile != null) {
                    adjacentMeadowZone.addAll(placedTile.meadowZones());
                }
            }
        }

        Area<Zone.Meadow> meadowArea = meadowArea(meadowZone);
        adjacentMeadowZone.retainAll(meadowArea.zones());

        return new Area<>(adjacentMeadowZone, meadowArea.occupants(), 0);
    }
